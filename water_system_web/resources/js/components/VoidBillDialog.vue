          ...
          <div class="w-full max-w-md rounded-lg shadow-lg transform transition-all duration-300 ease-out" :class="isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'">
            <div class="px-6 py-4 border-b" :class="isDark ? 'border-gray-800' : 'border-gray-200'">
              <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold">Void Bill</h3>
              ...
              <div class="mt-4 space-y-4">
                ...
                <div v-if="selectedReason === 'other' || selectedReason">
                  ...
                  <textarea
                    ...
                    rows="3"
                    class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 resize-none"
                    :class="isDark ? 'bg-gray-800 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'"
            ...
            <div class="px-6 py-4 flex items-center justify-end gap-3" :class="isDark ? 'bg-gray-800' : 'bg-gray-50'">
              ...
              <button type="button" @click="handleConfirm" :disabled="isLoading || !isValid" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium disabled:opacity-50 transition-colors duration-200">
                {{ isLoading ? 'Voiding...' : 'Void Bill' }}
              </button>
            </div>
          </div>
        </div>
...
export default {
  ...
  props: {
    ...
    message: {
      type: String,
      default: 'Are you sure you want to void this bill? This action cannot be undone.'
    },
    isLoading: {
      type: Boolean,
      default: false
  ...
  methods: {
    handleConfirm() {
      ...
      this.$emit('confirm', this.finalReason)
    },
    handleCancel() {
      this.$emit('cancel')
    }
